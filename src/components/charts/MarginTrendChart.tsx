...
export const MarginTrendChart: React.FC = () => {
    ...
    return years.map((year) => {
      ...
      if (isMultiCompany) {
        selectedCompanies.forEach((company) => {
          ...
          point[`opMargin_${company.id}`] = ratio?.operatingMargin ?? null;
        });
      } else {
        const ratio = selectedCompanies[0].ratios.find((r) => r.year === year);
        point.operatingMargin = ratio?.operatingMargin ?? null;
        point.netMargin = ratio?.netMargin ?? null;
      }
  ...
  return (
    <ChartWrapper title="Margin Trends">
          ...
          {isMultiCompany ? (
            selectedCompanies.map((company, idx) => (
              <Line
                key={company.id}
                type="monotone"
                dataKey={`opMargin_${company.id}`}
                name={`${company.name} Op. Margin`}
