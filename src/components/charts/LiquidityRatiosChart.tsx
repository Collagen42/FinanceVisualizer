...
export const LiquidityRatiosChart: React.FC = () => {
    ...
    return years.map((year) => {
      ...
      if (isMultiCompany) {
        selectedCompanies.forEach((company) => {
          ...
          point[`current_${company.id}`] = ratio?.currentRatio ?? null;
        });
      } else {
        const ratio = selectedCompanies[0].ratios.find((r) => r.year === year);
        point.currentRatio = ratio?.currentRatio ?? null;
        point.quickRatio = ratio?.quickRatio ?? null;
      }
  ...
  return (
    <ChartWrapper title="Liquidity Ratios">
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={chartData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
          ...
          <ReferenceLine
            ...
            stroke="#ef4444"
            strokeDasharray="4 4"
            label={{ value: 'Min (1.0)', position: 'right', fontSize: 10, fill: '#ef4444' }}
          />
          <ReferenceLine
            y={1.5}
            stroke="#10b981"
            strokeDasharray="4 4"
            label={{ value: 'Good (1.5)', position: 'right', fontSize: 10, fill: '#10b981' }}
          />
          {isMultiCompany ? (
