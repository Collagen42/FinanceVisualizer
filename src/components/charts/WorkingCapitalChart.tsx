...
export const WorkingCapitalChart: React.FC = () => {
    ...
    return years.map((year) => {
      ...
      if (isMultiCompany) {
        selectedCompanies.forEach((company) => {
          ...
          const ca = annual?.currentAssets;
          const std = annual?.shortTermDebts;
          point[`wc_${company.id}`] = ca != null && std != null ? ca - std : null;
        });
      } else {
        const annual = selectedCompanies[0].annuals.find((a) => a.year === year);
        const ca = annual?.currentAssets;
        const std = annual?.shortTermDebts;
        point.workingCapital = ca != null && std != null ? ca - std : null;
      }
  ...
  return (
    <ChartWrapper title="Working Capital">
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={chartData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
          ...
          {isMultiCompany ? (
            ...
            <Bar dataKey="workingCapital" name="Working Capital" radius={[2, 2, 0, 0]}>
              {chartData.map((entry, idx) => {
                const value = entry.workingCapital as number | null;
                return (
                  <Cell
                    key={idx}
                    fill={value != null && value >= 0 ? '#10b981' : '#ef4444'}
                  />
                );
              })}
            </Bar>
