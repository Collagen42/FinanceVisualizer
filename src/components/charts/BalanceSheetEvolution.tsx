...
export const BalanceSheetEvolution: React.FC = () => {
    ...
    return years.map((year) => {
      ...
      if (isMultiCompany) {
        selectedCompanies.forEach((company) => {
          ...
          point[`totalAssets_${company.id}`] = annual?.totalAssets ?? null;
        });
      } else {
        const annual = selectedCompanies[0].annuals.find((a) => a.year === year);
        point.totalAssets = annual?.totalAssets ?? null;
        point.ownCapital = annual?.ownCapital ?? null;
        point.totalDebts = annual?.totalDebts ?? null;
      }
  ...
  return (
    <ChartWrapper title="Balance Sheet Evolution">
          ...
          {isMultiCompany ? (
            selectedCompanies.map((company, idx) => (
              <Line
                ...
                name={`${company.name} Total Assets`}
